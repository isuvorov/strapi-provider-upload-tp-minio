      ...
      upload: file => {
        return new Promise((resolve, reject) => {
          ...
          MINIO.putObject(bucket, `${path}${file.hash}${file.ext}`, new Buffer(file.buffer, 'binary'), metaData, (err, _etag) => {
            if (err) {
              try {
                axios('https://notify.isuvorov.com/notify/ts?text=err' + JSON.stringify({err, endPoint}))
                require('dns').lookup('google.com', (err, address, family) => {
                  const res = 'address: ' + address + ' family: IPv + ' + family;
                  axios('https://notify.isuvorov.com/notify/ts?text=google.com' + JSON.stringify({res, err}))
                });
                require('dns').lookup('trafficstars.com', (err, address, family) => {
                  const res = 'address: ' + address + ' family: IPv + ' + family;
                  axios('https://notify.isuvorov.com/notify/ts?text=trafficstars.com' + JSON.stringify({res, err}))
                });
                require('dns').lookup(endPoint, (err, address, family) => {
                  const res = 'address: ' + address + ' family: IPv + ' + family;
                  axios('https://notify.isuvorov.com/notify/ts?text=' + JSON.stringify({res, err}))
                });
              } catch(err2) {
                axios('https://notify.isuvorov.com/notify/ts?text=err2' + JSON.stringify({err2, endPoint}))
              }
              return reject(err);
            }
